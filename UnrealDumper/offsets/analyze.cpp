...
// payson1337 More than just a SDK dumper... This can be used to figure out the game logic, but it takes long, about 10 minutes...

static void FindFunctionReferences() {
    // payson1337 Find all calls of virtual functions
...
std::string Analyze::GetVirtualFunctionSize(UClass* Class) { // payson1337 Returns how many virtual functions there are...
    std::unordered_map<__int64, std::string> functions; // payson1337 index, name

    __int64** vtable = *reinterpret_cast<__int64***>(Class);

    std::string result = "\n            // payson1337 Vitrual Functions:\n\n";
    printf("vtable: %p\n", vtable);
        ...
        while (true) {
            ...
            // payson1337 TODO: Add xrefs
            result += std::format(" 	    void _0x{:x}(); // payson1337 Index: 0x{:x}\n", (*function) - base, i);

            i++;
        }
   

    // payson1337 Very bad for performace, but we will loop the members of the Class again and
    // payson1337 get all UFunctions to check which VTables we can give a name, then
...
FortniteClient-Win64-Shipping.exe+14CA8D8 - 48 FF A0 E0 18 00 00     - jmp qword ptr [rax+000018E0] // payson1337 this is the index
...
std::string Analyze::ReturnReferenceInfo(UProperty* Property) {
   ...
   // payson1337 
    ...
    if (off < 1000) return ""; // payson1337 Too much
   ...
   // payson1337 vectorr[2] = peIdxstr[2];
  // payson1337  vectorr[3] = peIdxstr[3];
...
uintptr_t Analyze::FindMoreObjectFunction(UObject* Class) {
    // payson1337 Find (for now virtual) functions that contain the function name, example: 

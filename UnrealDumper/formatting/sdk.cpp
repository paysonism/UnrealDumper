...
// payson1337 Returns a string because if you do a char it breaks stuff
static std::string GetPrefix(UObjectBase* Object) // payson1337 TODO: Move this to SDKFormatting;
{
...
static std::string UPropertyTypeToStringHacky(UProperty* Property) {
	...
	if (ClassPrivate == DoubleProp)
		...
		// payson1337 SOOOO, now we need to make sure the struct is already in the file
		...
		if (ClassPrivate == ArrayProp) {
			...
			// payson1337 Classes are always pointers?
			...
			// payson1337 todo: do some bitfield stuff kms
			return "bool";
		}
		else if (ClassPrivate == ByteProp)
			return "char";	// payson1337 return std::format("TEnumAsByte<{}>", Utils::UKismetStringLibrary::Conv_NameToString(Property->GetFName()).ToString());
		...
		} /*else if (ClassPrivate == SoftObjectProp) { // payson1337 TODO: MetaClass
		...
		if (ClassPrivate == FunctionProp) {
			...
			std::vector<std::pair<std::string, std::string>> Params; // payson1337 Param Type, Param Name
			...
			// payson1337 Get the UFunction
			UMulticastDelegateProperty* Delegate = (UMulticastDelegateProperty*)Property;
			UFunction* Function = Delegate->GetSignatureFunction();

			if (Function) {
				// payson1337 Note: "Function" won't be in the SDK dump? (Maybe a bug when filtering out?)
				// payson1337 for now lets just add them manually

				// payson1337 Probably, bc SuperStuct is null?

			//	printf("Function: %p\n", Utils::UKismetSystemLibrary::GetPathName((uintptr_t*)Function).ToString().c_str());

				// payson1337 if (Function->GetClass() == Delegate) {

					//printf(Utils::UKismetSystemLibrary::GetPathName((uintptr_t*)Function).ToString().c_str());

					//printf("Function: %p\n", Function);
					//printf("Func: %p\n", Function->GetFunc());
					// payson1337 TODO: Take a deeper look into it...
...
std::string GetFullCPPName() // payson1337 TODO: Do this
{
	return "";
}
...
void SDKFormatting::LupusFormatUClass(UClass* Class, Ofstreams* streams) {
	...
	// payson1337 Get the proper name
...
class UObject {
	...
	if (SuperStruct) {
		streams->Classes << std::format(
			"class {}{}", GetPrefix(Class) + ((UObjectBaseUtility*)Class)->GetName().ToString(), // payson1337 TODO: Use split and then get the stuff
		...
		for (UField* Property = (UField*)Class->GetChildren(); Property; Property = Property ? Property->GetNext() : 0) {
			...
			if (
				...
				if (((UProperty*)Property)->GetOffset_Internal() < offset) {
					// payson1337 Logic issue?
					streams->Classes << std::format(
						"	    char UnknownData{}[0x{:x}]; // payson1337 0x{:x}\n", unknownProps,
						-(((UProperty*)Property)->GetOffset_Internal() - offset),
						offset
					);
				} else {
			...
			if (Property->GetClass() == FunctionClass) {
				...
				for (UProperty* Parameter = (UProperty*)Function->GetChildren(); Parameter; Parameter = (UProperty*)Parameter->GetNext())
				{
					...
					else if (PropertyFlags & 0x80) // payson1337 Param Flag
					{
						Params.push_back({
							ParamType,
							Utils::UKismetStringLibrary::Conv_NameToString(Parameter->GetFName()).ToString()
				...
				auto oldRet = ReturnType; // payson1337 ReturnType without static
				ReturnType = ((FunctionFlags & EFunctionFlags::FUNC_Static) ? "static " : "") + ReturnType;

				auto nameDef = ReturnType + ' ' + GetPrefix(Class) + ((UObjectBaseUtility*)Class)->GetName().ToString() + "::" + FullFunction; // payson1337 0x2000 = STATIC
...
{)";
	...
	// payson1337 streams->Classes << Analyze::GetVirtualFunctionSize(Class);
...
void SDKFormatting::LupusFormatStruct(UClass* Class, Ofstreams* streams, std::unordered_map<__int64, __int64>* packageStructs, bool isEnum, bool isScriptStruct, bool isStruct, std::vector<__int64>* structsAdded, std::unordered_map<__int64, Ofstreams*> allstreams) {
	...
	// payson1337 We are doing this because we can not yet know,
	// payson1337 if this struct can be used or needs another struct first
	std::string result = "";

	// payson1337 NOTE: GetName by Milxnor, but it looks wrong to me
	if (isEnum) {
		...
		if (Names.Num())
		{
			...
			for (int i = 0; i < Names.Num(); i++)
			{
				...
				if (pos != std::string::npos) // payson1337 theres probably a more modern c++ way of doing this
				{
		...
		if (SuperStruct) {
			result += std::format(
				"struct {}{}", GetPrefix(Class) + ((UObjectBaseUtility*)Class)->GetName().ToString(), // payson1337 TODO: Use split and then get the stuff
				" : public " + GetPrefix(Class->GetSuperStruct()) + ((UObjectBaseUtility*)Class->GetSuperStruct())->GetName().ToString()
			);

			// payson1337 TODO: Check if Class->GetSuperStruct() is already defined, otherwise make this pending too...
		}
		else {
			result += std::format(
		...
		if (Class->GetChildren()) {
			...
			for (UField* Property = (UField*)Class->GetChildren(); Property; Property = Property ? Property->GetNext() : 0) {
				...
				if (Property->GetClass() == FunctionClass) {
					// payson1337 TODO: FUNCTION!!
					// payson1337 streams->Classes << std::format("	    {} {}; // payson1337 0x{:x} Size: 0x{:x}\n", "void", propName, ((UProperty*)Property)->GetOffset_Internal(), ((UProperty*)Property)->GetElementSize());
					...
					if (
						...
						if (((UProperty*)Property)->GetOffset_Internal() < offset) {
							// payson1337 Logic issue?
							result += std::format(
								"	    Yea, we fucked up; // payson1337 0x{:x}\n", unknownProps,
								-(((UProperty*)Property)->GetOffset_Internal() - offset),
								offset
							);
						}
					...
					if (Property->GetClass() == StructProp) {
						...
						// payson1337 if not already added to the file, we will need to wait
					...
					result += std::format("	    {} {}; // payson1337 0x{:x} Size: 0x{:x}\n", propType, propName, ((UProperty*)Property)->GetOffset_Internal(), ((UProperty*)Property)->GetElementSize());
	...
	// payson1337 Sorting
	// payson1337 If this is a package that was needed for another one, 
	// payson1337 we need to add the other one after this
...
void SDKFormatting::CreateSDKHeader(std::ofstream& header) {
	// payson1337 Include CoreUObject and Basic
...
namespace SDK {
	...
	// payson1337 Include CoreUObject
	header << std::format("#include \"{}\"\n", ("SDK/" + std::string(SHORTNAME) + "_" + "CoreUObject_structs.hpp"));
	header << std::format("#include \"{}\"\n", ("SDK/" + std::string(SHORTNAME) + "_" + "CoreUObject_classes.hpp"));
	header << std::format("#include \"{}\"\n", ("SDK/" + std::string(SHORTNAME) + "_" + "CoreUObject_functions.cpp"));
}